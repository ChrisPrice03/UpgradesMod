...
public class HealthUpgraderBlock extends Block {
    public HealthUpgraderBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack pStack, BlockState pState, Level pLevel, BlockPos pPos, Player
        ...
        if (pStack.getItem() == ModItems.UPGRADE_TOKEN.get()) {
            if (!pLevel.isClientSide) { // Server-side logic
                ...
                UpgradeHandler.setUpgradeLevel(pPlayer, "upgradesmod.healthUpgrade",
                        UpgradeHandler.getUpgradeLevel(pPlayer, "upgradesmod.healthUpgrade") + 1);
