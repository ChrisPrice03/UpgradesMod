...
public class SpeedUpgraderBlock extends Block {
    public SpeedUpgraderBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack pStack, BlockState pState, Level pLevel, BlockPos pPos, Player
        ...
        if (pStack.getItem() == ModItems.UPGRADE_TOKEN.get()) {
            if (!pLevel.isClientSide) { // Server-side logic
                ...
                UpgradeHandler.setUpgradeLevel(pPlayer, "upgradesmod.speedUpgrade",
                        UpgradeHandler.getUpgradeLevel(pPlayer, "upgradesmod.speedUpgrade") + 1);
