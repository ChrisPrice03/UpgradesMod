...
public class SaturationUpgraderBlock extends Block {
    public SaturationUpgraderBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack pStack, BlockState pState, Level pLevel, BlockPos pPos, Player
        ...
        if (pStack.getItem() == ModItems.UPGRADE_TOKEN.get()) {
            if (!pLevel.isClientSide) { // Server-side logic
                ...
                UpgradeHandler.setUpgradeLevel(pPlayer, "upgradesmod.saturationUpgrade",
                        UpgradeHandler.getUpgradeLevel(pPlayer, "upgradesmod.saturationUpgrade") + 1);
