...
public class FortuneUpgraderBlock extends Block {
    public FortuneUpgraderBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack pStack, BlockState pState, Level pLevel, BlockPos pPos, Player
        ...
        if (pStack.getItem() == ModItems.UPGRADE_TOKEN.get()) {
            if (!pLevel.isClientSide) { // Server-side logic
                ...
                UpgradeHandler.setUpgradeLevel(pPlayer, "upgradesmod.fortuneUpgrade",
                        UpgradeHandler.getUpgradeLevel(pPlayer, "upgradesmod.fortuneUpgrade") + 1);
