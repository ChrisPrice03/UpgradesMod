...
public class TeleportationUpgraderBlock extends Block {
    public TeleportationUpgraderBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack pStack, BlockState pState, Level pLevel, BlockPos pPos, Player
        ...
        if (pStack.getItem() == ModItems.UPGRADE_TOKEN.get()) {
            if (!pLevel.isClientSide) { // Server-side logic
                ...
                UpgradeHandler.setUpgradeLevel(pPlayer, "upgradesmod.teleportationUpgrade",
                        UpgradeHandler.getUpgradeLevel(pPlayer, "upgradesmod.teleportationUpgrade") + 1);
