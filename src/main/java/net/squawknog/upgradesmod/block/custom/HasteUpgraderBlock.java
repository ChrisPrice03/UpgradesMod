...
public class HasteUpgraderBlock extends Block {
    public HasteUpgraderBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack pStack, BlockState pState, Level pLevel, BlockPos pPos, Player
        ...
        if (pStack.getItem() == ModItems.UPGRADE_TOKEN.get()) {
            if (!pLevel.isClientSide) { // Server-side logic
                ...
                UpgradeHandler.setUpgradeLevel(pPlayer, "upgradesmod.hasteUpgrade",
                        UpgradeHandler.getUpgradeLevel(pPlayer, "upgradesmod.hasteUpgrade") + 1);
